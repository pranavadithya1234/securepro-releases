...
<head>
    <meta charset="utf-8">
    <style>
        body { background-color: #0b0f19; color: #f3f4f6; font-family: system-ui, -apple-system, sans-serif; margin: 0; padding: 40px 20px; }
        .container { max-width: 600px; margin: 0 auto; background-color: #111827; border: 1px solid rgba(99, 102, 241, 0.2); border-radius: 16px; overflow: hidden; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.5); }
        .header { background: linear-gradient(135deg, #1e1b4b, #111827); padding: 30px; text-align: center; border-bottom: 1px solid rgba(99, 102, 241, 0.1); }
        .logo-badge { display: inline-block; width: 50px; height: 50px; line-height: 50px; background: linear-gradient(135deg, #6366f1, #8b5cf6); border-radius: 50%; font-size: 26px; margin-bottom: 12px; text-align: center; box-shadow: 0 0 15px rgba(139, 92, 246, 0.5); }
        .brand-name { font-size: 24px; font-weight: 800; color: #8b5cf6; letter-spacing: 1px; text-transform: uppercase; }
        .content { padding: 40px 30px; }
        .greeting { font-size: 18px; font-weight: 600; color: #ffffff; margin-bottom: 16px; }
        .message { font-size: 15px; line-height: 1.6; color: #9ca3af; margin-bottom: 30px; }
        .otp-card { background: rgba(99, 102, 241, 0.05); border: 1px solid rgba(99, 102, 241, 0.3); border-radius: 12px; padding: 24px; text-align: center; margin-bottom: 35px; box-shadow: inset 0 2px 4px rgba(0, 0, 0, 0.2); }
        .otp-label { font-size: 12px; font-weight: 700; color: #6366f1; text-transform: uppercase; letter-spacing: 2px; margin-bottom: 10px; }
        .otp-code { font-family: 'Courier New', Courier, monospace; font-size: 38px; font-weight: 800; color: #8b5cf6; letter-spacing: 6px; margin: 0; text-shadow: 0 0 10px rgba(139, 92, 246, 0.3); }
        .brand-story { border-top: 1px solid rgba(255, 255, 255, 0.05); padding-top: 30px; margin-top: 30px; }
        .story-title { font-size: 16px; font-weight: 700; color: #ffffff; margin-bottom: 12px; }
        .story-text { font-size: 13px; line-height: 1.6; color: #9ca3af; margin-bottom: 20px; }
        .feature-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 24px; }
        .feature-item { background: rgba(255, 255, 255, 0.02); border: 1px solid rgba(255, 255, 255, 0.05); border-radius: 8px; padding: 12px; font-size: 12px; color: #d1d5db; }
        .feature-icon { color: #6366f1; margin-right: 6px; font-weight: bold; }
        .footer { background-color: #0d1117; padding: 20px; text-align: center; font-size: 11px; color: #4b5563; border-top: 1px solid rgba(255, 255, 255, 0.03); }
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
            ...
            <div class="brand-name">SecurePro AI</div>
        </div>
        <div class="content">
            <div class="greeting">Hello, ${studentName}</div>
            <div class="message">
                Please use the following One-Time Password (OTP) to log in to your secure assessment portal. This code is valid for 10 minutes. Do not share this OTP with anyone.
            </div>
            <div class="otp-card">
            ...
            <div class="brand-story">
                <div class="story-title">SecurePro AI Platform</div>
                <div class="story-text">
                    SecurePro AI is a next-generation, AI-powered examination proctoring platform designed to ensure the integrity, transparency, and security of remote assessments.
                </div>
                <div class="feature-grid">
                    <div class="feature-item"><span class="feature-icon">✓</span> AI Exam Monitoring</div>
                    <div class="feature-item"><span class="feature-icon">✓</span> Suspicious Detection</div>
                    <div class="feature-item"><span class="feature-icon">✓</span> Secure Desktop Env</div>
                    <div class="feature-item"><span class="feature-icon">✓</span> Auto Secure Updates</div>
                </div>
            </div>
        </div>
        <div class="footer">
            © 2026 SecurePro AI. All rights reserved. &bull; GDPR Compliant &bull; ISO 27001
        </div>
    </div>
</body>
</html>`;
            ...
            const mailOptions = {
                from: '"SecurePro AI" <securepro.kmit@gmail.com>',
                to: targetEmail,
                subject: 'SecurePro AI - OTP Verification',
                html: htmlContent
            };
            ...
            console.warn('[EMAIL] Nodemailer import/init failed, falling back to EmailJS:', nodemailerErr);
            fallbackToEmailJS();
        }
    ...
    try {
        for (const gid of selectedGroups) {
            ...
            for (const sid of group.students) {
                ...
                const isNew = !current.includes(eid);
                if (isNew) {
                ...
                // Send invitation email to ALL students writing the exam
        ...
        if (total > 0 || skipped > 0) {
            toast(`Assigned ${total} new student${total !== 1 ? 's' : ''}. Invitation emails sent to all ${total + skipped} student${(total + skipped) !== 1 ? 's' : ''}!`);
        }
...
<head>
    ...
    <style>
        ...
        .greeting { font-size: 18px; font-weight: 600; color: #ffffff; margin-bottom: 16px; }
        .message { font-size: 15px; line-height: 1.6; color: #9ca3af; margin-bottom: 30px; }
        .invite-card { background: rgba(99, 102, 241, 0.05); border: 1px solid rgba(99, 102, 241, 0.3); border-radius: 12px; padding: 24px; margin-bottom: 35px; }
        .invite-title { font-size: 12px; font-weight: 700; color: #6366f1; text-transform: uppercase; letter-spacing: 2px; margin-bottom: 15px; text-align: center; }
        .detail-row { display: flex; justify-content: space-between; margin-bottom: 10px; font-size: 14px; border-bottom: 1px solid rgba(255, 255, 255, 0.03); padding-bottom: 8px; }
        .detail-label { color: #6b7280; font-weight: 500; }
        .detail-value { color: #ffffff; font-weight: 600; }
        .btn-portal { display: block; background: linear-gradient(135deg, #6366f1, #8b5cf6); color: #ffffff !important; text-align: center; padding: 14px 24px; border-radius: 8px; text-decoration: none; font-weight: 700; font-size: 15px; margin: 25px 0 10px; box-shadow: 0 4px 12px rgba(99, 102, 241, 0.3); }
        .brand-story { border-top: 1px solid rgba(255, 255, 255, 0.05); padding-top: 30px; margin-top: 30px; }
        .story-title { font-size: 16px; font-weight: 700; color: #ffffff; margin-bottom: 12px; }
        .story-text { font-size: 13px; line-height: 1.6; color: #9ca3af; margin-bottom: 20px; }
        .feature-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 24px; }
...
<body>
    <div class="container">
        ...
        <div class="content">
            <div class="greeting">Hello, ${student.name || 'Student'}</div>
            <div class="message">
                You have been assigned a new examination on the SecurePro AI platform. Please review the schedule and details below and log in to your student application to complete the assessment.
            </div>
            <div class="invite-card">
                <div class="invite-title">Examination Details</div>
                ...
                <div class="detail-row">
                    ...
                    <span class="detail-value" style="font-size: 12px; max-width: 60%; text-align: right;">${windowInfo}</span>
                </div>
                <a href="#" class="btn-portal">Open SecurePro Application</a>
            </div>
            <div class="brand-story">
                <div class="story-title">SecurePro AI Platform</div>
                <div class="story-text">
        ...
        <div class="footer">
            ...
            const mailOptions = {
                from: '"SecurePro AI" <securepro.kmit@gmail.com>',
                to: student.email,
                subject: 'SecurePro AI - New Exam Invitation',
                html: htmlContent
            };
            ...
            console.warn('[EMAIL] Nodemailer import failed, falling back to EmailJS:', nodemailerErr);
            fallbackToEmailJS().then(resolve).catch(reject);
        }
    });
}
